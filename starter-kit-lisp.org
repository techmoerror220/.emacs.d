...
#+RESULTS:
| (lambda nil (define-key racket-mode-map (kbd C-c C-r) 'racket-run)) | (lambda nil (define-key racket-mode-map (kbd C-c r) 'racket-run)) | run-starter-kit-coding-hook | racket-unicode-input-method-enable |

** Geiser

Trying to use racket-mode by default.
