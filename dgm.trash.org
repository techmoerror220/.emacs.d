...
;; (diminish 'linum-relative-mode)
  ;; DW: give some breathing room. New setting as of 14
    ;; march 21 overriding previous one in init.el
     ;; I include this blink-cursor-mode again because sometimes it disappears and maybe turning this fixes it.
...
#+RESULTS:
: #s(hash-table size 65 test eql rehash-size 1.5 rehash-threshold 0.8125 data (:use-package (24561 62018 558019 123000) :init (24561 62018 557974 419000) :config (24561 62018 557306 773000) :config-secs (0 0 2150 397000) :init-secs (0 0 3362 929000) :use-package-secs (0 0 3601 602000)))
...
#+begin_src emacs-lisp :tangle no
 ...
 ;; Redshift
   (defvar efs/redshift-process nil
    "Holds the process of the running Redshift instance, if any")

  (defun efs/kill-redshift ()
    (interactive)
    (when efs/redshift-process
      (ignore-errors
        (setq efs/redshift-process (start-process-shell-command "redshift" nil "redshift -x"))))
    (setq efs/redshift-process nil))

  (defun efs/start-redshift ()
    (interactive)
    (efs/kill-redshift)
    (setq efs/redshift-process (start-process-shell-command "redshift" nil "redshift -O 3500 -m randr")))
